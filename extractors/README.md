...
| `.ts`, `.tsx`, `.mts`, `.cts` | `ts.py` | `parse`, `ENTRY_RULES` (tree-sitter-typescript front-end, grammar split ts/tsx per suffix: `.tsx` adds JSX patterns; overloads collapse to the first declaration line with the implementation body; barrel re-export rebinding to origin definers, tsconfig `paths` alias resolution with JSONC tolerance + one `extends` level, dynamic `import()`/`require()` module liveness, decorators + exported PascalCase React components + module-scope render targets → entry_hints, get/set accessor merge; extensionless specifiers resolve across the ts/js suffix sets both directions (`JS_EXTS` ships here, js.py imports it); `TS_BASE_VIRTUALS` + mention floor feed the dead tier) |
| `.js`, `.jsx`, `.mjs`, `.cjs` | `js.py` | `parse`, `ENTRY_RULES` (tree-sitter-javascript front-end, `.jsx` under the tsx grammar; CJS `require`/`module.exports` beside ESM with interop defaults; ESM + CJS barrels wiring-only with origin rebinding; jsconfig/tsconfig alias resolution; exported PascalCase React components, render targets and `*.stories.*` are entry roots; mixed-repo specifiers resolve across the ts/js suffix sets both directions; React class lifecycle virtuals + mention floor feed the dead tier) |
| `.rs` | `rust.py` | `parse`, `ENTRY_RULES` (tree-sitter-rust front-end; `fn main` + `#[test]`/`#[tokio::test]` entry hints — a bare `#[cfg(test)]` gate marks nothing; lib.rs pub-fn + `pub mod` closure are the exported-API roots, so `pub` alone in a non-exported mod stays dead-eligible; `pub use` re-exports rebind consumers to origin definers; impl/trait overload collapse to the first declaration; trait default-method dispatch + `_emit_call` override mirror; `mod` namespaces resolve crate-rooted paths; macros recorded as call-sites in name_literals, never expanded) |
| `.cs` | `csharp.py` | `parse`, `ENTRY_RULES` (tree-sitter-c-sharp front-end; static `Main` + `[Test]`/`[TestMethod]`/`[Fact]`/`[Theory]` attribute hints + Unity MonoBehaviour lifecycle methods (Awake/Start/Update/…) as convention entries on Unity-based files, mirroring the Godot `_ready` rooting; partial-class declarations merge to one flat surface, first declaration wins the line; namespace + `using` facts with namespace-qualified call resolution via a type index (wider than class_map: every declared type, not just the file header) + namespace-prefix walk; no .sln/.csproj in v1 — loud degrade; #if preproc bodies descend at file and member level; this-writes from `this.X =` sites; no import liveness sweep by design — a C# using is name resolution, not a module binding, liveness flows through the call edges only) |
