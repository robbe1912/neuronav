...
| module | role |
|---|---|
| `__init__.py` | registry: `EXTENSIONS` maps suffix -> module (`.gd`/`.tscn` -> `gdscript`, `.py` -> `python`, `.c` -> `c`, `.h`/`.hpp`/`.cpp`/`.cc`/`.cxx` -> `cpp`, `.ts`/`.tsx`/`.mts`/`.cts` -> `ts`, `.js`/`.jsx`/`.mjs`/`.cjs` -> `js`, `.rs` -> `rust`, `.go` -> `go`, `.java` -> `java`, `.cs` -> `csharp`, `.php` -> `php`, `.lua` -> `lua`); `registry_for(suffix)` returns module or None; `RAW_TEXT_EXTS` = the issue-#240 web set with no structural extractor (`.json .md`) — walked/indexed as raw `file_doc` when configured (TS went structural in #245, JS in #277); `PRESETS` = the `onboard.py init --preset <key>` extension lists — the 12 keys ts/js/python/cpp/c/gdscript/rust/go/java/csharp/php/lua, derived from the registry since #362 (each key = that module's registry suffixes in insertion order + `RAW_TEXT_EXTS`; a new registry suffix flows into every preset walking that language; only the c row stays hand-curated — `.h` pairs with `.c`, `.hpp` stays cpp's — and cpp deliberately walks no raw text; import-time parity assert aborts on a registry module with no preset key; the ts preset carries `.cjs` too since #277) |
