...
| module | role |
|---|---|
| `__init__.py` | registry: `EXTENSIONS` maps suffix -> module (`.gd`/`.tscn` -> `gdscript`, `.py` -> `python`, `.c` -> `c`, `.h`/`.hpp`/`.cpp`/`.cc`/`.cxx` -> `cpp`, `.ts`/`.tsx`/`.mts`/`.cts` -> `ts`, `.js`/`.jsx`/`.mjs`/`.cjs` -> `js`, `.rs` -> `rust`, `.go` -> `go`, `.java` -> `java`, `.cs` -> `csharp`, `.php` -> `php`); `registry_for(suffix)` returns module or None; `RAW_TEXT_EXTS` = the issue-#240 web set with no structural extractor (`.json .md`) — walked/indexed as raw `file_doc` when configured (TS went structural in #245, JS in #277); `PRESETS` = the `onboard.py init --preset ts\|js\|python\|cpp\|c\|gdscript\|rust` extension lists (curated; the ts preset carries `.cjs` too since #277) |
