...
config that walks nothing is a typo, not an empty index. The one
exception is the server BOOT walk (issue #240): it degrades to
first-call guidance (naming the extensions scanned + a paste-ready
config for the suffixes actually on disk) so the MCP handshake still
completes; the explicit rescan path stays fatal.
...
| `extensions` | `.gd, .tscn` | suffixes kept (registered ones parse structurally; the rest — the #240 raw-text web set `.ts .tsx .js .jsx .mjs .mts .cts .json .md`, scaffolded by `onboard.py init [--preset ts\|js\|python\|cpp\|gdscript]` — index as raw `file_doc`: searchable, but find_functions/symbol_graph/dead_code return nothing for them until extractors land) |
