...
from extractors import go as go_x  # noqa: E402
...
RUST_ROOTS = set(rust_x.RUST_STD_TRAIT_METHODS)
GO_ROOTS = set(go_x.GO_INIT_VIRTUALS) | set(go_x.GO_STD_IFACE_METHODS)
...
for fam in ("verifier", "pyhard", "cpp", "mwires", "ts", "rust", "go"):
...
# pass 2: evaluate
fam_files = {".py": set(), "gd": set(), "cpp": set(), "ts": set(), "rs": set(),
             "go": set()}
...
for rel, fs, sites, ext, goals in CORPUS:
    ...
    fam_files["gd" if fs.ext in (".gd", ".tscn") else "cpp" if is_cpp
              else "ts" if fs.ext in ts_x.TS_EXTS
              else "rs" if fs.ext in rust_x.RUST_EXTS
              else "go" if fs.ext in go_x.GO_EXTS else ".py"].add(rel)
    for no, goal in goals:
        ...
        if dm:
            ...
            roots = (CPP_ROOTS if is_cpp else TS_ROOTS if fs.ext in ts_x.TS_EXTS
                     else RUST_ROOTS if fs.ext in rust_x.RUST_EXTS
                     else PY_ROOTS if fs.ext == ".py"
                     else GO_ROOTS if fs.ext in go_x.GO_EXTS else GD_ROOTS)
