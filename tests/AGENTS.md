...
Forty-seven self-contained suites. Each is a standalone script — no pytest — run in
its own process:
...
CI (`.github/workflows/ci.yml`) runs thirty-eight hermetic suites on ubuntu
with `NEURONAV_EMBED_FAKE=1` (`test_strata`, `test_crosslang`,
`test_pyhard`, `test_cpphard`, `test_jshard`, `test_gohard`, `test_javahard`, `test_chard`, `test_phphard`, `test_autorescan`, `test_server_stdio`,
