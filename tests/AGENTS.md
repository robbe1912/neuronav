...
| `test_javahard` | Java extractor edge cases (issue #335: package-path import resolution incl. static-member/wildcard + external loud-degrade, main(String[]) + JUnit entry rules, interface default-method dispatch + @Override review shielding, qualified `new` ctor edges + default-ctor name-level alive, dead tiers, double-build determinism, FAKE end-to-end); rides the shared `tests/harness.py` check/finish (issue #372) | tree-sitter + tree-sitter-java wheels (hermetic fixtures under `fixtures/java`, Maven-style layouts) |
...
| `test_onboardprogress` | onboarding observability (issue #315): the first index build and the viz bake never read as a dead server — `neuronav://onboarding/status` resource advertised + readable mid-build, a progressToken'd rescan streams `notifications/progress`, visualize acks "bake accepted" immediately with the bake landing via the background baker, stale reads engage only past the grace window (`stale: true` banner), and a body SystemExit surfaces unwrapped from the async shell; the progressToken leg's pins ride an in-flight status-resource poller (readiness evidence, no wall-clock floor — issue #401); rides the shared `tests/harness.py` check/finish (issue #372) | mcp + chromadb (hermetic temp fixture + config, `NEURONAV_EMBED_FAKE=1`; server-under-test selectable via argv for pre-fix FAIL evidence) |
| `test_navsplit` | nav split smokes (issue #344): the three leaves carry focused legs — config precedence + the rebind-propagation law (attribute reads propagate to every leaf after `use_config`), embed dims follow the live config, rescan walk dedupe, `config_scope` exact-restore across leaves, store-lock singleton identity across alternation, clusters memo identity, CLI trailing-argv rejection survival (#332), and the atomic-migration law itself (AST scan: zero `nav.X` attribute refs left in production modules; extractors stay nav-leaf-import-light); rides the shared `tests/harness.py` check/finish (issue #372) | hermetic scratch corpus + FAKE embeds |
| `test_chromabounds` | bounded full-collection reads (issue #327): the four whole-store chroma gets (rescan existing-rows, re-stamp, base export, bake embeddings) page through `navstore.col_get_all` under the caller's lock instead of one unfiltered `col.get()` that dies with InternalError "too many SQL variables" past the sqlite build's ceiling — teeth sized to the real ceiling (33k-row direct-add store rescans green; 32.7k just-under control), bounded read equals the unfiltered get id-sorted, base export stays byte-identical across two runs, re-stamp copy lands count-intact and sorted; #402 leg H: every read primitive (bare counts, paged gets) rides the bounded hnsw-settle retry — injected real-signature teeth: fail-N-1 recovers row-identical with a per-attempt stderr log, beyond-bound fails LOUD with the original error after the bounded attempt count, wrong-signature errors stay immediate, and the GK-authorized rider pins the routed first-contact gate (`server._first_contact`) onto `navstore.count()` — a bare collection count reverted into the gate fails the suite | chromadb import (hermetic temp fixture, `NEURONAV_EMBED_FAKE=1`; checkout-under-test selectable via `--nav-dir` argv for pre-fix FAIL evidence) |
...
| `test_clusterinv` | cluster partition invariant + crosstalk parity (issue #114): finalize's family moves vs full-weld regroups can double-assign a file — repaired by weld plurality (last pass, identity on healthy input); crosstalk counts only wiring the clusterer's structural graph sees (tests/ endpoints tallied separately, no cluster number); #393 legs: topk_desc stays byte-identical to the argsort reference `np.argsort(-sim, axis=1)[:, :k]` on tie-heavy corpora (alphabet/banded/sub-chunk × k=1,6,n-1) while its transient allocations stay chunk-bounded (tracemalloc peak pin; the monolithic rows·n form blew past CI's 16GB at 33k tie-heavy docs), double-run digest stability | numpy + chromadb import only (crafted shapes + stub graph, self-sets `NEURONAV_EMBED_FAKE=1`) |
...
| `test_embedprov` | embed provider contract (issue #17): provider select/auto-detect, ollama+openai wire adapters, env-vs-config key precedence, keyless no-header, 401 loud, 429 retry, batch chunking, no-pad mismatches, fake-mode isolation, pre-#17 store heal vs provider-drift refusal + raw-provider messages (#159); #402: the stamp/heal windows (collection open, `_adopt_orphan` probe counts, `_restamp` copy-count) ride the bounded hnsw-settle retry — injected transient recovers with records identical and the retry named on stderr | stdlib http.server stub on an ephemeral loopback port + chromadb import |
| `test_project_mode` | onboarding (issue #27): discovery precedence env > project-local > checkout, `onboard.init`/`wire` scaffolds incl. `.neuroignore` (issue #36), init re-run NEVER clobbers a customized config (issue #121), wire BOM-tolerant + loud on malformed MCP jsons + atomic writes (issue #121), `wire --omp` mcpServers fragment shape (issue #130: default/`--omp-name` server names, merge-preserving writes, two-project no-collision, hermetic `NEURONAV_OMP_MCP` reroute), viz add-on degrade, `PRESETS` registry-derivation pins (issue #362: byte-equal equivalence vs the hand-spelled table it replaced, key-order pin for server.py's suggestion preference + servercore's guidance string, a new registry suffix flows into its preset on re-derivation, a registry module with no preset key aborts loud naming it) + the `server._auto_rescan` rebind seam tooth proving late-bound family rails (issue #359, store-free via `search_text("")`'s empty-pattern arm) | stdlib + chromadb import (fresh subprocesses, fake embeds) |
...
| `test_bytelaws` | byte-level output laws (issue #124): `_importmap` pinned against the five vendored files (CRLF→LF embed law, relative-specifier rewrite, determinism); `_ADDONS` derived from the joined template (#370 teeth: template keys == the pinned import surface, first-occurrence order live, a new template import joins the set, an unvendored specifier is refused loudly, a dropped import leaves it); UTF-8-no-BOM law asserted on every generated JSON artifact (onboard config scaffold, export_base manifest, baked graph.html DATA/importmap splices) | chromadb + numpy import (hermetic temp config + FAKE store, self-sets `NEURONAV_EMBED_FAKE=1`) |
