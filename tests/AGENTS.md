...
Thirty-four self-contained suites. Each is a standalone script — no pytest — run in
its own process:
...
CI (`.github/workflows/ci.yml`) runs twenty-nine hermetic suites on ubuntu
...
`test_bootgate`, `test_impact`) plus a `viz` job that builds
