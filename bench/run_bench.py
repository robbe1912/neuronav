...
def _sweep_table(recs: dict[str, dict]) -> list[str]:
    ...
    lines = [
        ...
        "both rank lists enter with src=graph). `gb` below pins the winner;",
        "baseline `both` = λ 0 at the shipped k=60. Deterministic grid, every",
        "cell double-run — wins inside the documented Ollama ±jitter are treated",
        "as ties.",
        "",
        "| config | hit@1 | hit@5 | hit@10 | MRR | reach@5 | reach@10 |",
        "|---|---|---|---|---|---|---|",
