...
The finalize pass pipeline (blob split + label passes, issue #380 carve)
moved verbatim to clusterpasses.py; finalize and the pass names tests
pin are re-exported below — partition bytes identical across the carve.
"""
...
from clusterpasses import (  # noqa: E402  (#380 carve: the finalize pass
    # pipeline lives in the leaf; this facade re-export keeps every
    # pinned name — navstore's finalize, the tests' pass constants —
    # resolving unchanged)
    PART_CAP,
    PART_SOFT_CAP,
    _pack_key,
    _pass_partition,
    _pass_scene_majority,
    _stem,
    finalize,
)
